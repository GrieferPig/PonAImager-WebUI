...
parser.add_argument("--steps", type=int,
                    help='iteration steps, default to 50', default=50)
...
parser.add_argument("--oa",
                    help='Enable attention-slicing', action="store_true")
parser.add_argument("--os",
                    help='Enable sequential-cpu-offload', action="store_true")
parser.add_argument("--ov",
                    help='Enable vae-slicing', action="store_true")
parser.add_argument("--ox",
                    help='Enable xformers (require a xformers install)', action="store_true")
parser.add_argument("--device", type=str,
                    help='set inference device, default to cpu', default="cpu")
parser.add_argument("--modelId", type=str,
                    help='Model ID/Path', default="", required=True)
parser.add_argument("--revision", type=str,
                    help='Branch of the Model to be used, default to main', default="main")
parser.add_argument("--proxy", type=str,
                    help='Proxy used to download model', default="")
parser.add_argument("--disableChecker",
                    help='Disable nsfw checker', action="store_true")
...
renderParser.add_argument("--scale", type=float,
                          help='scale, default to 7.5', default=7.5)
renderParser.add_argument("--steps", type=int,
                          help='iteration steps, default to 50', default=50)
renderParser.add_argument("--height", type=int,
                          help='image height, must be multiples of 32, default to 512', default=512)
renderParser.add_argument("--width", type=int,
                          help='image width, must be multiples of 32, default to 512', default=512)
renderParser.add_argument("--seed", type=int,
                          help='Specify a seed for generation', default=-1)
renderParser.add_argument("--outname", type=str,
                          help='set image filename', default="", required=True)
renderParser.add_argument('prompt', type=str, nargs='*',
                          help='Prompts to generate image, 77 token(word) limit')
renderParser.add_argument('--negative', type=str, nargs='*', default="",
                          help='Negative prompt: Will not be considered by AI while generating img, 77 token(word) limit')
...
println("Using model "+model_id + " revision " +
        args.revision + " running on " + device)
...
if args.listen:
    ...
    if args.outname == "":
        ...
        println("Oh and by the way, my dear, you may specify what name the image should be by using --outname yourfrickingimage.png, and it will appear in the temp folder after a while.")
        sys.exit(101)  # lol
    prompt = " ".join(args.prompt)
    neg = " ".join(args.negative)
    draw(prompt, args.scale, args.steps,
