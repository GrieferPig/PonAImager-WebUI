...
function updateRenderingTask(proc_output: Buffer){
    ...
    let _begin = _output.indexOf("<");
    let _end = _begin + 5;
    _task.estiTime = +_output.substring(_begin, _end);
    
    _begin = _output.indexOf(",  ");
    _end = _output.indexOf("]");
    _task.iterSpeed = _output.substring(_begin, _end);

    taskList.set(renderingUUID, _task);
}

function concatArg(req: RenderStat): string{
    let _arg = `--scale ${req.origReq.scale} --steps ${req.origReq.steps} --height ${req.origReq.height} --width ${req.origReq.width} --sampler ${req.origReq.sampler} --seed ${req.origReq.seed} --outname ${req.uuid} ${req.origReq.prompt}`;
    if(!MEM_OPT){
        _arg += " --noopt";
    }
    _arg += os.EOL;
